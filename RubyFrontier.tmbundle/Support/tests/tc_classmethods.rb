  ...
  describe "class methods" do
    describe "guaranteePageOfSite" do
      it "raises for non-existent page" do
        expect{
          UserLand::Html.guaranteePageOfSite @nonexistentpage 
        }.must_raise RuntimeError
      end
      it "raises for non-page object" do
        expect{
          UserLand::Html.guaranteePageOfSite @template 
        }.must_raise RuntimeError
      end
      it "does nothing for page object" do
        expect{
          UserLand::Html.guaranteePageOfSite @firstpage 
        }.must_be_silent
      end
    end
    describe "getFtpSiteFile" do
      it "raises for file with no ftpsite file above it" do
        expect{ 
          UserLand::Html.getFtpSiteFile __FILE__ 
        }.must_raise RuntimeError
      end
      it "returns the ftpsite file's pathname" do
        ftpsite = @folder + "#ftpSite.yaml"
        expect(
          UserLand::Html.getFtpSiteFile(@template)
        ).must_equal ftpsite # non-page object
        expect(
          UserLand::Html.getFtpSiteFile(@firstpage)
        ).must_equal ftpsite # page object
        expect(
          UserLand::Html.getFtpSiteFile(@deeperpage)
        ).must_equal ftpsite # deeper page object
        expect(
          UserLand::Html.getFtpSiteFile(@nonexistentpage)
        ).must_equal ftpsite # non-existent object but folder exists
      end
    end
    describe "everyPageOf" do
      ...
      describe "everyPageOfFolder" do
        it "lists every page object at all depths down only" do
          expect(
            simpleOrdered(:everyPageOfFolder, @folder)
          ).must_equal %w{firstpage fourthpage secondpage thirdpage}
          expect(
            simpleOrdered(:everyPageOfFolder, @deeperpage.dirname)
          ).must_equal %w{ fourthpage }
        end
      end
      describe "everyPageOfSite" do
        it "lists every page object at all depths up and down" do
          expect(
            simpleOrdered(:everyPageOfSite, @firstpage)
          ).must_equal %w{firstpage fourthpage secondpage thirdpage}
          # folder ok
          expect(
            simpleOrdered(:everyPageOfSite, @deeperpage.dirname)
          ).must_equal %w{firstpage fourthpage secondpage thirdpage}
          # nonpage object ok
          expect(
            simpleOrdered(:everyPageOfSite, @template)
          ).must_equal %w{firstpage fourthpage secondpage thirdpage}
          expect(
            simpleOrdered(:everyPageOfSite, @deeperpage)
          ).must_equal %w{firstpage fourthpage secondpage thirdpage}
        end
      end
    end
    describe "getLink" do
      ...
      it "uses linetext and url" do
        expect(
          UserLand::Html.getLink("biteme", "url")
        ).must_equal %{<a href="url">biteme</a>}
      end
      it "uses linetext, url, arbitrary options hash" do
        ...
        expect(
          atag.attributes.count
        ).must_equal 3
        expect(
          atag.children[0].text
        ).must_equal 'biteme'
      end
    ...
    describe "preflightsite and autoglossary structure" do
      before do
        unless @@preflighted # do once
          ...
          @@autog = File.open( @folder + '#autoglossary.yaml' ) { |yf| YAML.load_file(yf, permitted_classes: [Pathname, Symbol], aliases: true) }
          @@preflighted = true
        end
      end
      it "autoglossary represents a hash containing exactly these 8 entries" do
        ...
        titles.each { |title| expect(@@autog[title]).wont_be_nil}
      ...
      it "autoglossary values have the correct structure" do
        ...
        expect(
          fp[:linetext]
        ).must_equal "My Fourth Web Page"
        expect(
          fp[:path]
        ).must_equal Pathname("folder/fourthpage.html")
        expect(
          fp[:adr]
        ).must_equal @deeperpage
      end
    end
  end
...
END
  ...
  describe "releaseRenderedPage" do
    ...
    it "raises for a nonpage object" do
      expect{
        UserLand::Html.releaseRenderedPage(@template, false, false)
      }.must_raise RuntimeError
    end
  ...
  describe "publishSite" do
    it "builds site1 correctly" do
      ...
      out, err = capture_io do # suppress output
      ...
      expect(
        out
      ).must_match %r%finished publishing%i # fails sometimes and I'm not sure why
