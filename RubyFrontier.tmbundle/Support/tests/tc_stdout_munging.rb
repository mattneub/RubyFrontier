...
describe RubyFrontier::FakeStdout do
  it "replaces newline with br" do
    ...
    expect(
      out
    ).must_equal 'howdy<br>there<br>I\'m Oedipus Tex<br>You may have heard of my brother Rex<br>'
  end
  it "turns line-end single-quoted string starting with slash to txmt link" do
    ...
    expect(
      out
    ).must_equal 'this is a <a href="txmt://open?url=file:///cool/test">/cool/test</a><br>'
    expect(
      out
    ).must_match %r%txmt%
    out, err = capture_io do
    ...
    expect(
      out
    ).wont_match %r%txmt% # because it wasn't line-final
    out, err = capture_io do
      RubyFrontier::FakeStdout.open do
        puts "this is a \"/cool/test\""
      end
    ...
    expect(
      out
    ).wont_match %r%txmt% # because it wasn't single-quoted
    out, err = capture_io do
      RubyFrontier::FakeStdout.open do
        puts "this is a 'cool/test'"
      end
    ...
    expect(
      out
    ).wont_match %r%txmt% # because there's no initial slash
  end
  it "nicely formats exception reports" do
    ...
    expect(
      out
    ).must_match %r%^oops<br>%
    expect(
      out
    ).must_match %r{RubyFrontier.tmbundle/Support/tests/tc_stdout_munging.rb:\d*:in 'test_raise'<br>}
  end
end
...
describe "perform" do
  it "with true, wraps pre with FakeStdout around any output" do
    ...
    expect(
      out
    ).must_match %r{(<pre>\s*|<br>)this is a test<br>and this is a test<br></pre>}
    ...
    expect(
      out
    ).must_match %r{RubyFrontier.tmbundle/Support/bin/RubyFrontier/longestJourney/userland_class_methods.rb:\d*:in 'UserLand::Html.test_raise'<br>}
  end
